...
(eval-when (:compile-toplevel :load-toplevel :execute)
  ...
  (defun symb (&rest args)
    (unless args
      (error "symb needs args"))
    (intern (format nil "~{~A~}" args) (symbol-package (first args)))))
...
(defmacro conflog-regist (gadget init-status &body body)
  `(progn
     ...
     (set-status (symb (gadget-id ,gadget) '.value) ,init-status)
     (set-status (symb (gadget-id ,gadget) '.status) 'on)
...
(defmacro conflog-gen-prologue-rules ()
  `(progn ,@(loop for pred in (reverse *pred*)
	       collect `(:- (,pred ?value) (or (^ ,(symb pred '.value)) (^ ,(symb pred '.status)))
			    ...
			    (,(symb pred '.status) ?status) (or (and (== ?status disable) (support ,pred on) (show ,pred on) (enable ,pred off))
								(or (and (== ?status hide) (support ,pred on) (show ,pred off))
								    (or (and (== ?status unsupport) (support ,pred off))
									(and (== ?status on) (support ,pred on) (show ,pred on) (enable ,pred on)))))))))

(defmacro conflog-gen-epilogue-rules ()
  `(progn ,@(loop for pred in (reverse *pred*)
	       append `((:- (,(symb pred '.value) ?value) (,pred ?value))
			;; (:- (,(symb pred '.status) ?status) (or (and (support ,pred ?off) (== ?off off) (= ?status unsupport))
			;; 					(or (and (show ,pred ?off) (== ?off off) (= ?status hide))
...
(defvar *unsupport* (make-hash-table))
(conflog::define-primitive support/2 (pred status cont)
  (let ((g (gethash pred *id/gadget* nil)))
    (when g
      (if (conflog::unbound-var-p status)
	  (conflog::unify! status (if (sheet-grafted-p g) 'on 'off))
	  (case (intern (symbol-name (conflog::deref status)) :keyword)
	    (:off (when (sheet-parent g)
		    (setf (gethash g *unsupport*) (cons g (sheet-parent g)))
		    (sheet-disown-child (sheet-parent g) g)))
	    (:on (ignore-errors
		   (when (gethash g *unsupport* nil) ;This is tricky and even buggy :(
		     (setf (frame-current-layout (pane-frame g)) (frame-current-layout (pane-frame g)))
		     (unless (sheet-parent g)
		       (sheet-adopt-child (cdr (gethash g *unsupport*)) g))
		     (setf (frame-current-layout (pane-frame g)) (frame-current-layout (pane-frame g)))
		     (setf (gethash g *unsupport*) nil)
		     (loop for u being each hash-value in *unsupport*
			when (and u (sheet-parent (car u)))
			do (sheet-disown-child (sheet-parent (car u)) (car u))))))))))
  (funcall cont))
